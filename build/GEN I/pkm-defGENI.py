...
def build_score_map(entry):
    ...
    using keys: 'r' (resistances), 'w' (defensive resistances),
    'oi' (defensive immunities), 'n' (normal effectiveness).
    ...
    # Resistances
    resistances_list = entry.get('r', [])
    for matchup in resistances_list:
        tgt = matchup['type']
        val = matchup['score']
        score_map[tgt] = val

    # Wesknesses
    weakness_list = entry.get('w', [])
    for matchup in weakness_list:
        tgt = matchup['type']
        val = matchup['score']
        score_map[tgt] = val

    # Immunities
    immunity_list = entry.get('i', [])
    for matchup in immunity_list:
        tgt = matchup['type']
        val = matchup['score']
        score_map[tgt] = val
...
def calculate_defensive_score(requested_type, data):
    """
    Steps:
    ...
    if '/' in requested_type:
        ...
        type_names = [requested_type]
    debug(f"    Types determined: {type_names}")
    ...
    if len(score_maps) == 1:
        # Monotype: use score map directly
        combined_score_map = score_maps[0]
    else:
        map1, map2 = score_maps[0], score_maps[1]
        for tgt in all_target_types:
            combined = 1.0
            for smap in score_maps:
                combined *= get_score(smap, tgt)
            combined_score_map[tgt] = combined
            debug(f"    Target='{tgt}', Combined Multiplier={combined}")
    ...
    debug("Step 9: Summing final defensive score")
    total_score = 0.0
    ...
    debug(f"Final Defensive Score for {requested_type}: {total_score}\n")
    return total_score
...
def handle_csv_export(data):
    try:
        ...
        monotype_combinations = [(types, calculate_defensive_score('/'.join(types), data)) for types in itertools.combinations(type_ids, 1)]
        dualtype_combinations = [(types, calculate_defensive_score('/'.join(types), data)) for types in itertools.combinations(type_ids, 2)]
        tripletype_combinations = [(types, calculate_defensive_score('/'.join(types), data)) for types in itertools.combinations(type_ids, 3)]
        output_dir = os.path.dirname(os.path.abspath(__file__))
        export_combinations_to_csv(monotype_combinations, os.path.join(output_dir, 'monotype_combinations_defence.csv'), ['First Type', 'Defensive Score', 'Defensive Score', 'Total Score'])
        print("Monotype combinations successfully exported to monotype_combinations_defence.csv")
        export_combinations_to_csv(dualtype_combinations, os.path.join(output_dir, 'dualtype_combinations_defence.csv'), ['First Type', 'Second Type', 'Defensive Score', 'Defensive Score', 'Total Score'])
        print("Dual type combinations successfully exported to dualtype_combinations_defence.csv")
        export_combinations_to_csv(tripletype_combinations, os.path.join(output_dir, 'tripletype_combinations_defence.csv'), ['First Type', 'Second Type', 'Third Type', 'Defensive Score', 'Defensive Score', 'Total Score'])
        print("Triple type combinations successfully exported to tripletype_combinations_defence.csv")
...
def interactive_prompt(data):
    while True:
        ...
        if user_input.lower() == 'quit':
            ...
            try:
                score = calculate_defensive_score(user_input, data)
                print(f"Defensive Score for {user_input}: {score}")
            except Exception as e:
                print(f"Error: {e}")

if __name__ == "__main__":
    script_dir = os.path.dirname(os.path.abspath(__file__))
    pkm_score_path = os.path.join(script_dir, "pkm-scoreGENI.json")
